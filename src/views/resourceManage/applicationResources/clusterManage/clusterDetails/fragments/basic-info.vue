...
<template>
  <div class="px-4">
    <div>
      <div class="my-4">集群基本信息</div>
      <div class="bg-[#f7f8fa] p-4 rounded-sm">
        <a-descriptions :column="2">
          <a-descriptions-item label="名  称"> {{ basicInfo.name }}</a-descriptions-item>
          <a-descriptions-item label="env">{{ basicInfo.env }}</a-descriptions-item>
          <a-descriptions-item label="关联应用">{{
            basicInfo.app && basicInfo.app.name
          }}</a-descriptions-item>
          <a-descriptions-item label="关联主机">
            <span v-for="(item, index) in basicInfo.hosts" :key="index">
              {{ item.ip }}{{ basicInfo.hosts.length - 1 != index ? '，' : '' }}
            </span>
          </a-descriptions-item>
...
<script lang="ts" setup>
  ...
  import { getClusterDetails } from '@/api/resourceManage/applicationResources/clusterManage';
